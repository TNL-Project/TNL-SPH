...
def generate_moving_square_boundary_particles(setup):
    ...
    for l in range(0, n_layers):
        for i in range(0 - l, nx + l):
            x = i * dp
            box_rx.append(x)
            box_ry.append(0.0 - dp * l)
            normal_x.append(0.0)
            normal_y.append(1.0)
    ...
    ny = int(round(H / dp))
    for l in range(0, n_layers):
        for j in range(1 - l, ny + l):
            y = j * dp
            box_rx.append(0.0 - l * dp)
            box_ry.append(y)
            normal_x.append(1.0)
            normal_y.append(0.0)
            particle_type.append(0)
    ...
    for l in range(0, n_layers):
        for j in range(0 - l, ny + l + 1):
            y = j * dp
            box_rx.append(L + l * dp)
            box_ry.append(y)
            normal_x.append(-1.0)
            normal_y.append(0.0)
            particle_type.append(0)
    ...
    for l in range(0, n_layers):
        for i in range(0 + l, nx_side - l):
            x = x_square - half_D + i * dp
            y = y_square - half_D + l * dp
            box_rx.append(x)
            box_ry.append(y)
            normal_x.append(0.0)
            normal_y.append(-1.0)
    ...
    for l in range(0, n_layers):
        for j in range(1 + l, nx_side - l):
            x = x_square - half_D + l * dp
    ...
    #for j in range(nx_side + 1):
    for l in range(0, n_layers):
        for j in range(0 + l, nx_side - l + 1):
            x = x_square + half_D - l * dp
            y = y_square - half_D + j * dp
            box_rx.append(x)
            box_ry.append(y)
    ...
    #    offset = layer * dp
    #    inner_half_D = half_D - offset

    #    # Skip this layer if offset too large (inner_half_D <= 0)
    #    if inner_half_D <= 0:
    #        continue

    #    inner_D = 2 * inner_half_D
    #    nx_inner = int(round(inner_D / dp))

    #    # Only add points if nx_inner >= 2 (allows at least one interior point per side with range(1, nx_inner))
    #    if nx_inner < 2:
    #        continue

    #    # Bottom inner side
    #    for i in range(0, nx_inner):
    #        x = x_square - inner_half_D + i * dp
    #        y = y_square - inner_half_D
    #        box_rx.append(x)
    #        box_ry.append(y)
    #        normal_x.append(0.0)
    #        normal_y.append(-1.0)
    ...
    #    # Left inner side
    #    for j in range(0, nx_inner):
    #        x = x_square - inner_half_D
    #        y = y_square - inner_half_D + j * dp
    #        box_rx.append(x)
    #        box_ry.append(y)
    #        normal_x.append(-1.0)
    #        normal_y.append(0.0)
...
if __name__ == "__main__":
    ...
    g.add_argument("--dp", type=float, default=0.02, help="initial particle spacing (default corresponds to ny≈400 in original script)")
    g.add_argument("--h-coef", type=float, default=2**0.5, help="smoothing length coefficient h = h_coef * dp")
    ...
    g.add_argument("--density", type=float, default=1000, help="reference fluid density")
    g.add_argument("--speed-of-sound", type=float, default=28, help="speed of sound (multiples of U)")
    g.add_argument("--cfl", type=float, default=0.1, help="CFL number")
    g.add_argument("--alpha", type=float, default=0.00001, help="artificial viscosity alpha")
    ...
    g.add_argument("--viscous-term", type=str, default="ArtificialViscosity", help="viscosity formulation")
