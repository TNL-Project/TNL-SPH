...
wallNormal_x = []; wallNormal_y = []; wallNormal_z = []
...
for layer in range( numberOfBoundaryLayers ):
    for x in range( wallL_n ):
        ...
        wallNormal_x.append( 0. )
        wallNormal_y.append( 0. )
        wallNormal_z.append( 1. )
...
if write == '.ptcs':
    ...
    n = np.array( ( wallNormal_x, wallNormal_z, wallNormal_y ), dtype=float ).T #!!
    v = np.zeros( ( len( wall_rx ), 3 ) )
    rho = rho0 * np.ones( len( wall_rx ) )
    p = np.zeros( len( wall_rx ) )
    ptype = np.ones( len( wall_rx ) )

    wallToWrite = saveParticlesVTK.create_pointcloud_polydata( r, v, rho, p, ptype, n )
...
gridYbegin = 1.7 * ( ( min( min( fluid_rz ), min( wall_rz ) ) ) - searchRadius )
