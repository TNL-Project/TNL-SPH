...
public:
   ...
   writeInfo() noexcept;
    ...
    /*
       ...
       set. The geometry is static, fixed at init by one of two variants: the
       interpolation variant (useInterpolation == true) reads an own-resolution wall
       lattice pre-generated by the init script from a boundary-ghost-buffer-<p> file;
       the copy variant (default) appends band-selected copies of the neighbor boundary
       particles. In both variants rho and gamma are refreshed from the source boundary
       every step in interact(). srcIndexList is used only by the copy variant and holds
       referential indices (identity at init) so the mapping survives the boundary
       sorting performed by the neighbor search at step 0.
    */
