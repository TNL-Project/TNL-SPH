...
template< typename FluidPointer,
          ...
          typename EOS,
          typename SPHState >
typename WCSPH_DBC< ParticleSystem, SPHFluidConfig, Variables >::RealType
WCSPH_DBC< ParticleSystem, SPHFluidConfig, Variables >::interactionWithReduction( FluidPointer& fluid,
                                                                                  BoudaryPointer& boundary,
                                                                                  SPHState& sphState )
{
   /* PARTICLES AND NEIGHBOR SEARCH ARRAYS */
   ...
   auto FluidFluid = [=] __cuda_callable__ ( LocalIndexType i, LocalIndexType j,
         VectorType& r_i, VectorType& v_i, RealType& rho_i, RealType& p_i, RealType* drho_i, VectorType* a_i, RealType *maxVisco ) mutable
   {
      const VectorType r_j = view_points[ j ];
      const VectorType r_ij = r_i - r_j;
      const RealType drs = l2Norm( r_ij );
      if (drs <= searchRadius )
      {
         ...
         *drho_i += ( v_ij, gradW ) * m - diffTerm;
         ...
         const RealType p_term = ( p_i + p_j ) / ( rho_i * rho_j );
         const RealType dot_r_ij_v_ij = ( r_ij, v_ij );
         const RealType visco =  ViscousTerm::Pi( rho_i, rho_j, drs, dot_r_ij_v_ij, viscousTermTermsParams );
         *a_i += ( -1.0f ) * ( p_term + visco ) * gradW * m;

         *maxVisco = std::max( dot_r_ij_v_ij, *maxVisco );
      }
   };
   ...
   auto particleLoop = [=] __cuda_callable__ ( LocalIndexType i ) mutable
   {
      ...
      RealType maxVisco_i = 0.f;

      NeighborsLoop::exec( i, r_i, searchInFluid, FluidFluid, v_i, rho_i, p_i, &drho_i, &a_i, &maxVisco_i );
      NeighborsLoop::exec( i, r_i, searchInBound, FluidBound, v_i, rho_i, p_i, &drho_i, &a_i );
