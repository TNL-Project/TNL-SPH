...
//#include "../../../SPHTraits.h"
//#include <SPH/shared/MidpointSchemeOutputLog.h>
...
class MidpointWithAndersonIntegrationSchemeVariables
{
   public:
   ...
   MidpointWithAndersonIntegrationSchemeVariables() = default;
...
template< typename SPHConfig >
class MidpointSchemeWithAnderson
{
public:
   ...
   using IntegrationSchemeVariablesType = MidpointWithAndersonIntegrationSchemeVariables< SPHConfig >;
   ...
   predictor( RealType dt, FluidPointer& fluid )
   {
      ...
      // Anderson acceleration keeps a short history of the (a, drho) iterates
      // across the upcoming sub-iteration loop. That history is only valid
      // for *this* outer time step (the fixed point itself changes once
      // r_in/v_in/rho_in change), so it must be rebuilt here, every predictor
      // call - regardless of whether AA ends up being used below, this is
      // cheap (a couple of reductions).
      andersonAccelerator.reset( fluid->getParticles(), fluid->getIntegratorVariables()->dvdt_in,
                                  fluid->getIntegratorVariables()->drhodt_in );
   }
   ...
   // Plain damped Picard relaxation (original scheme):
   //    a_{k+1} = relaxMidpoint * a_in + ( 1 - relaxMidpoint ) * a_new
   template< typename FluidPointer, typename ModelParams >
   void
   relaxPicard( FluidPointer& fluid, ModelParams& modelParams )
   {
   ...
   // Anderson-accelerated relaxation: same inputs/outputs as relaxPicard()
   // (reads dvdt_in/drhodt_in as x_k and the just-computed a/drho as g(x_k),
   // writes the extrapolated next iterate back into a/drho), but uses the
   // last few iterations of history instead of only the latest one.
   template< typename FluidPointer, typename ModelParams >
   void
   relaxAnderson( FluidPointer& fluid, ModelParams& modelParams )
   {
      andersonAccelerator.mix( fluid->getParticles(),
                                fluid->getIntegratorVariables()->dvdt_in, fluid->getIntegratorVariables()->drhodt_in,
                                fluid->getVariables()->a, fluid->getVariables()->drho );
   }

   // Dispatches to relaxAnderson() or relaxPicard() depending on
   // modelParams.useAndersonAcceleration, so existing call sites
   // (integrator->relax(fluid, modelParams)) don't need to change at all.
   template< typename FluidPointer, typename ModelParams >
   void
   relax( FluidPointer& fluid, ModelParams& modelParams )
   {
      if( modelParams.useAndersonAcceleration )
         relaxAnderson( fluid, modelParams );
      else
         relaxPicard( fluid, modelParams );
   }
   ...
   runMidpointSubiteration( ModelParams& modelParams )
   {
      if( midpointIteration == 0 ){
         ...
         // pick up the Anderson-acceleration knobs from modelParams once per
         // outer time step (cheap, just scalars); see control.h for the
         // matching config entries.
         andersonAccelerator.configure( modelParams.andersonHistoryDepth,
                                        modelParams.andersonBeta,
                                        modelParams.andersonRegularization,
                                        modelParams.andersonGammaSafeguard );
      }
   ...
   AndersonAccelerator< SPHConfig > andersonAccelerator;
};
