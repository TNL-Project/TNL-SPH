...
template< typename FluidPointer, typename OpenBoudaryPointer >
void
WCSPH_DBC< Particles, ModelConfig >::interactionWithBoundaryPatches( FluidPointer& fluid,
                                                                     OpenBoudaryPointer& boundaryPatch,
                                                                     ModelParams& modelParams )
{
   // searchable object
   typename Particles::NeighborsLoopParams searchInOpenBoundary( boundaryPatch->particles );
   ...
   const auto view_points_bound = boundaryPatch->particles->getPoints().getView();
   auto view_rho_bound = boundaryPatch->variables->rho.getView();
   auto view_v_bound = boundaryPatch->variables->v.getView();

   const auto zoneParticleIndices_view = boundaryPatch->zone.getParticlesInZone().getConstView();
   const GlobalIndexType numberOfZoneParticles = boundaryPatch->zone.getNumberOfParticles();
...
void
WCSPH_DBC< Particles, ModelConfig >::computePressureFromDensity( PhysicalObjectPointer& physicalObject, ModelParams& modelParams )
{
...
void
WCSPH_DBC< Particles, ModelConfig >::interactWithPeriodicBoundary( FluidPointer& fluid,
                                                                   BoudaryPointer& boundary,
                                                                   ...
                                                                   const VectorType shift )
{
   ...
   auto particleLoop = [=] __cuda_callable__ ( LocalIndexType i ) mutable
   {
      ...
      TNL::ParticleSystem::NeighborsLoop::exec( p, r_i, searchInFluid, FluidFluid, v_i, rho_i, p_i, &drho_i, &a_i );
...
//          typename std::enable_if_t< std::is_same_v< BCType, WCSPH_BCTypes::MDBC >, bool > Enabled >
//void
//WCSPH_DBC< Particles, ModelConfig >::updateSolidBoundaryPeriodicBoundary( FluidPointer& fluid,
//                                                                          BoudaryPointer& boundary,
//                                                                          OpenBoundaryPointer& openBoundary,
//                                                                          ModelParams& modelParams,
//                                                                          const VectorType shift )
...
//      const RealType p_i = EOS::DensityToPressure( rho_i, eosParams );
//      const VectorType ghostNode_i = view_ghostNode_bound[ p ];
//
//      RealType drho_i = 0.f;
//      Matrix A_gn = 0.f;
...
//            p, ghostNode_i, searchInOpenBoundary, BoundOpenBoundary, v_i, rho_i, p_i, &A_gn, &b_gn );
//
