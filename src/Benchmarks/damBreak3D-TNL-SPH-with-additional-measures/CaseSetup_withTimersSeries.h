...
int main( int argc, char* argv[] )
{
   ...
   /**
    * Collect computation time of every operation in every step.
    * Here, we define fields, to store this information.
    * Timers "timer_pressure" and "timers_seacrh_reset" are ommited, since these are not interesting.
    */
   using TimerArrayType = TNL::Containers::Array< float, TNL::Devices::Host >;

   //Assuming constant time step, compute size of timers arrays:
   int stepsTotal = ceil( simulationControl.endTime / timeStepping.getTimeStep() );

   TimerArrayType timer_search_array( stepsTotal ), timer_interact_array( stepsTotal ), timer_search_sort_array( stepsTotal ), \
                  timer_total_array( stepsTotal );

   while( timeStepping.runTheSimulation() )
   {
      std::cout << "Time: " << timeStepping.getTime() << std::endl;
      float totalTime_step_init = ( timer_search.getRealTime() + \
       ...
       */
      float timer_search_step_init = timer_search.getRealTime();
      float timer_search_sort_step_init = timer_search_sort.getRealTime();
      timer_search.start();
      ...
      timer_search.stop();
      float timer_search_step = timer_search.getRealTime() - timer_search_step_init;
      float timer_search_sort_step = timer_search_sort.getRealTime() - timer_search_sort_step_init;
      std::cout << "Search... done. " << std::endl;

      /**
       * Perform interaction with given model.
       */
      float timer_interact_step_init = timer_interact.getRealTime();
      ...
      sph.template interact< SPHParams::KernelFunction, SPHParams::DiffusiveTerm, SPHParams::ViscousTerm, SPHParams::EOS >(
            sphParams );
      timer_interact.stop();
      float timer_interact_step = timer_interact.getRealTime() - timer_interact_step_init;
      ...
      /**
       * Collect time steps into timer arrays.
       */
      float timer_totalTime_step = ( timer_search.getRealTime() + timer_interact.getRealTime() + timer_integrate.getRealTime() + \
                                      timer_pressure.getRealTime() ) - totalTime_step_init;

      timer_interact_array.setElement( timeStepping.getStep(), timer_interact_step );
      timer_search_array.setElement( timeStepping.getStep(), timer_search_step );
      timer_search_sort_array.setElement( timeStepping.getStep(), timer_search_sort_step );
      timer_total_array.setElement( timeStepping.getStep(), timer_totalTime_step );
   ...
   /**
    * Output arrays with timers.
    * TODO: Array is saved into binary form using TNL::Containers::Array I/O functions.
    * I would rather see this arrays in ASCII.
    */
   //interaction timer from every timestep
   std::ofstream timer_interact_array_file;
   timer_interact_array_file.open( "results/timer_array-time_interact_array.dat" );
   for( int i = 0; i < timer_interact_array.getSize(); i++ )
   {
      timer_interact_array_file << i << "," << timer_interact_array.getElement( i ) << std::endl;
   }
   timer_interact_array_file.close();

   //search timer from every timestep
   std::ofstream timer_search_array_file;
   timer_search_array_file.open( "results/timer_array-time_search_array.dat" );
   for( int i = 0; i < timer_search_array.getSize(); i++ )
   {
      timer_search_array_file << i << "," << timer_search_array.getElement( i ) << std::endl;
   }
   timer_search_array_file.close();

   //search-sort timer from every timestep
   std::ofstream timer_search_sort_array_file;
   timer_search_sort_array_file.open( "results/timer_array-time_search_sort_array.dat" );
   for( int i = 0; i < timer_search_sort_array.getSize(); i++ )
   {
      timer_search_sort_array_file << i << "," << timer_search_sort_array.getElement( i ) << std::endl;
   }
   timer_search_sort_array_file.close();

   //total timer from every timestep
   std::ofstream timer_total_array_file;
   timer_total_array_file.open( "results/timer_array-time_total_array.dat" );
   for( int i = 0; i < timer_total_array.getSize(); i++ )
   {
      timer_total_array_file << i << "," << timer_total_array.getElement( i ) << std::endl;
   }
   timer_total_array_file.close();
