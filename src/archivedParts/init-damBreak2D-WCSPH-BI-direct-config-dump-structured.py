...
def generate_dam_break_fluid_particles( dp, rho0, speed_of_sound, fluid_length, fluid_height ):
    fluid_rx = []; fluid_ry = []
    fluid_density = []
...
def generate_dam_break_boundary_particles( dp, rho0, box_length, box_height, n_boundary_layers ):
    box_rx = []; box_ry = []
    ghost_rx = []; ghost_ry = []
    normal_x = []; normal_y = []
    ...
    return boundary_n, min( box_rx ), min( box_ry ), max( box_rx ), max( box_ry )

def compute_domain_size( search_radius, domain_origin_x, domain_origin_y, domain_end_x, domain_end_y ):
    eps = 1.005
    eps_sloshing = 1.2
    domain_origin_x = eps * ( domain_origin_x - search_radius )
    domain_origin_y = eps * ( domain_origin_y - search_radius )
    ...
    return domain_origin_x, domain_origin_y, domain_size_x, domain_size_y
...
if __name__ == "__main__":
    ...
    # compile-time parameters (config.h — C++ template type selection)
    compile_time_config = {
    ...
    # generate particles
    fluid_n = generate_dam_break_fluid_particles(
        args.dp, args.density, args.speed_of_sound, fluid_length, fluid_height )
    boundary_n, domain_origin_x, domain_origin_y, domain_end_x, domain_end_y = generate_dam_break_boundary_particles(
        args.dp, args.density, box_length, box_height, n_boundary_layers )

    # compute domain size
    search_radius = 2 * h_coef * args.dp
    domain_origin_x, domain_origin_y, domain_size_x, domain_size_y = compute_domain_size(
        search_radius, domain_origin_x, domain_origin_y, domain_end_x, domain_end_y )

    # assemble config with organizational subsections (solver flattens them automatically)
    config = {
        "control": {
        ...
        "particles": {
            "numberOfParticles": fluid_n,
            "numberOfAllocatedParticles": fluid_n,
            "numberOfBoundaryParticles": boundary_n,
            "numberOfAllocatedBoundaryParticles": boundary_n,
        },
        "domain": {
            "searchRadius": search_radius,
            "domainOrigin-x": domain_origin_x,
            "domainOrigin-y": domain_origin_y,
            "domainSize-x": domain_size_x,
            "domainSize-y": domain_size_y,
        },
        "model": {
            "dp": args.dp,
            "h": h_coef * args.dp,
    ...
    # dump directly to config.jsonc (subsections are flattened by the solver)
    save_params_to_json( config, 'sources/config.jsonc' )
