...
def run_single_test(log_path: Path, ref_path: Path, tol: float = 1e-9,
    ...
    try:
        ...
        fail_msg(f'ould not load reference JSON: {e}')
        return False
...
def main():
    ap = argparse.ArgumentParser(
        description='SPH log tester — generate reference JSONs or run comparisons',
    ...
    tol = args.tol if args.tol != 1e-9 else global_tol  # CLI flag overrides
